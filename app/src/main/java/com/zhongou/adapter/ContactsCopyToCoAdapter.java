 ...
 * 抄送 通讯录 适配
 */
public class ContactsCopyToCoAdapter extends BaseAdapter implements SectionIndexer{

	private List<ContactsSonCOModel> list = null;
	private Context mContext;
	private static HashMap<Integer,Boolean> isSelectedMap;//用来控制CheckBox的选中状况

	public ContactsCopyToCoAdapter(Context mContext, List<ContactsSonCOModel> list) {
	 ...
	 * @param list
	 */
	public void updateListView(List<ContactsSonCOModel> list){
		this.list = list;
	...
	public View getView(final int position, View convertView, ViewGroup arg2) {
		...
		final ContactsSonCOModel mContent = list.get(position);
		if (convertView == null) {
		...
		viewHolder.tvTitle.setText(this.list.get(position).getsStoreName());
...
//	}
	...
	public static void setIsSelectedMap(HashMap<Integer, Boolean> isSelected) {
		ContactsCopyToCoAdapter.isSelectedMap = isSelected;
	}
}